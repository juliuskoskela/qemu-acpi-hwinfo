...
in
{
  ...
  config = mkIf cfg.enable {
    # MicroVM integration
    microvm = mkIf cfg.enableMicrovm {
      qemu.extraArgs = mkIf (builtins.pathExists cfg.hostHwinfoPath) [
        "-acpitable"
        "file=${cfg.hostHwinfoPath}"
      ];
    };

    # QEMU integration for regular VMs
    virtualisation.qemu = mkIf cfg.enableQemuIntegration {
      options = mkIf (builtins.pathExists cfg.hostHwinfoPath) [
    ...
    environment.systemPackages = mkIf cfg.guestTools (with pkgs; [
      acpica-tools
      pciutils
      usbutils
    ]);
